	...
	char *machine, *user;
{
	...
	if ((icon = fopen(iconpath(machine, user), "r")) == NULL &&
	    (icon = fopen(iconpath(UNKNOWNMACH, user), "r")) == NULL &&
	    (icon = fopen(iconpath(machine, UNKNOWNUSER), "r")) == NULL &&
	    (icon = fopen(iconpath(UNKNOWNMACH, UNKNOWNUSER), "r")) == NULL)
		iconp=(char *)blankface;
	else{
		iconp=IconBuf;
		for (i=0; i<ICONBYTES && c!=EOF; i+=2) {
			while ((c=getc(icon))!='x' && c!=EOF)
					;
			val = HexToDec(getc(icon))<<4;
			val |= HexToDec(getc(icon));
			IconBuf[i]=val;
			val = HexToDec(getc(icon))<<4;
			val |= HexToDec(c=getc(icon));
			IconBuf[i+1]=val;
		}
		fclose(icon);
	}
	sendjerq('I', iconp, ICONBYTES);
}
