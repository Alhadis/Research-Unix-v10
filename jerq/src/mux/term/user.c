...
Uown()
{
	register got=CPU|cansend(P-proctab);
	if(P->state&GOTMOUSE)
		got|=MOUSE;
	...
	int fc;
{
	...
	for (; c = *s++; p.x += i->width) {
		...
		q.y = p.y+r.origin.y;
		if (b->someobs == b->nonevis)
			bitblt(f->bits,r,b->nonevis?b->obs:(Bitmap *)b,q,fc);
		else
	...
	int fc;
{
	...
	unsigned char *cp,mrbuf[2];
	int dw;
	short cwid,edge;
	Fontchar *infop;
	if (b->someobs != 0 || f != &defont || fc != F_XOR 
	...
	Rectangle r;
{
	...
	if(l->someobs != l->nonevis)
		...
		rectf(l->nonevis?l->obs:(Bitmap *)l, r, f);
	if(inhibited)
		cursallow();
}
Usegment(l, p, q, f)
	...
	Texture *t;
{
	...
	if(l->someobs != l->nonevis)
		...
		texture(l->nonevis?l->obs:(Bitmap *)l, r, t, f);
	if(inhibited)
		cursallow();
}
Uscreenswap(b, r, s)
...
#define Nalloc(n)	(struct Nqueue *)realalloc(sizeof(struct Nqueue)+(n)-3, (char *)0)

static struct Nqueue *nlist;

struct Nqueue *
getname(name)
register char *name;
{
	register struct Nqueue *p = nlist;
	while (p && strcmp(name, p->name))
		p = p->next;
	if (p == 0) {
		if ((p = Nalloc(strlen(name))) == 0)
			return 0;
		p->next = nlist;
		nlist = p;
		strcpy(p->name, name);
...
char *name; long data;
{
	register struct Nqueue *p = getname(name);
	if (p == 0)
		return 1;
	p->proc = P;
	p->data = data;
