...
#define F_COPY		((Code)5)	/* put source on top of target	*/
...
#define Spraying()	(Spray.value&SPRAYBIT)
#define Continuous()	(Spray.value&CONTBIT)
#define Dashing()	( Spray.value&DASHBIT )
#define DashOn()	(DashIndex==0)
extern Word BrushSize;
extern Word DashIndex;
...
/*	flags returned by Image generating functions	*/
...
typedef
struct Image	{
			Bitmap		*bp;	/* to hold the image	*/
			Bitmap		*mask;	/* for F_COPY mode	*/
			Rectangle	r;
			Point		pos;
			Point		alg;
		} Image;

extern Image MakeImage();
