...
register char *argv[];
{
	...
	(void)ioctl(1, TIOCGDEV, &ttydsave);
#else
	...
	if(!mpx){
#if vax | u3b | u3b2
		maxpktdsize = min(sizes[ttysave.sg_ospeed & 017], (long)MAXPKTDSIZE);
...
/**** ask gerard: ***/ gstats(fopen("/usr/gerard/32stat", "a"), file);
	if(psflag)
		pstats(stderr);
	return(0);
}
