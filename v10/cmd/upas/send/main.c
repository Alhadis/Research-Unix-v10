	...
	int pipeto;
{
	...
	file = s_new();
	mbox = s_to_c(dp->addr);
	if (strncmp (mbox, "local!", 6) == 0)
		mbox += 6;
	abspath(mbox, MAILROOT, file);
	next = d_rm_same(&dp);
	...
	if (pipeto) {
		...
		alarm(PIPETIMELIMIT);	/* don't let a user hold us up. */
	}
	...
	if (status != 0) {
		if (timedout)
			s_append(errstr, "'Pipe to' command took too long");
	...
	char *cp;
{
	...
	s_append(errstr, "' (");
	s_append(errstr, s_to_c(mp->replyaddr));
	s_append(errstr, ")");
