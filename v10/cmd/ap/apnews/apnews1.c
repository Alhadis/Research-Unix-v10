...
while (gets(line))
	...
	switch(line[0])
		...
		case 'C': /* copy file to my directory */
			...
			nviewed+= 3; /* big bonus if they copy it */
			break;
		case 'D': /* delete file*/
			i = atoi(line+1);
			if (i>0)
...
fgets(line, 100, f);
trimnl(line);
if (line[1]==' ' && line[5]==' ' && line[8]=='-') /* their heading */
	{
	for(p=line+9; (c = *p) && c!=' '; p++)
		*s++ = c;
	*s=0;
	return;
	}
...
for(i=0; i<10; i++)
	{
	char xheads[50], yheads[50], *scr();
	clrline();

	if (scoring)
		{
		if (heads[i][0])
		sprintf(xheads, "%.20s %s", heads[i], scr(names[i]));
		else
		xheads[0]=0;
		if (heads[i+10][0])
		sprintf(yheads, "%.20s %s", heads[i+10], scr(names[i+10]));
		else
		yheads[0]=0;
		}
	else
		{
		strncpy(xheads, heads[i], 20);
		strncpy(yheads, heads[i+10], 20);
		xheads[20]= yheads[20]=0;
		}
	if (strlen(names[i])<=5)
...
for (i=0; i<n; i++)
	{
		...
		if ((names[i][0]=='a'||names[i][0]=='b'||names[i][0]=='f')
		 && isdigit(names[i][1]))
			continue;
...
if (r>10) return;
if (neval[n]>nread[n] || neval[n]>tscor[n] || tscor[n]>6*neval[n])
	neval[n]=tscor[n]=sqscr[n]=0;
nread[n]++;
if (r>0)
	{
	neval[n]++;
