...
{
if((int *)p<=mem || (int *)p>mem+nmemused)
	{
...
else	return( (int *)ALLOC(exprblock) );
}
...
else	return( (int *)ALLOC(execblock) );
}
...
for(p = *p0 ; p->nextp ; p = (chainp)p->nextp)
	p->datap = 0;
...
for(i=0 ; i<nmemused ; )
	...
	fprintf(diagfile, "tag %3d", ((struct headbits *)mem)[i+1].tag);
	if(((struct headbits *)mem)[i+1].tag==TNAME && ((struct defblock *)mem)[i+1].sthead!=0)
		fprintf(diagfile, "   varname %s", ((struct stentry *)((struct defblock *)mem)[i+1].sthead)->namep);
	else if(j==2)
		fprintf(diagfile, "  chain %o %o", mem[i+1], mem[i+2]);
	else if (((struct headbits *)mem)[i+1].tag > TIOSTAT)
		{
		char *s, *sn;
		s = (char *)& mem[i+1];
		sn = s + 12;
