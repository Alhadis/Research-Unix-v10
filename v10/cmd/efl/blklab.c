...
chainp p;
register ptr q;
register struct stentry *v, *s;
struct stentry **hp;

for(hp = hashtab ; hp<hashend ; ++hp)
	if(s = *hp)
		{
		if( (v = (struct stentry *)s->varp) && v->blklevel == blklevel)
			{
			if(v->tag==TLABEL)
				if(blklevel <= 1)
					{
...
for( p=hidlist  ;  p && ((v = ((struct stentry *)((struct defblock *)(q=p->datap))->sthead))->varp==NULL) ; p=hidlist )
	{
	v->varp = q;
	v->tag = q->tag;
	v->subtype = q->subtype;
...
/*		if(thisexec->brnchend == 0)	*/
		...
		deflabno = 0;
		first = YES;
		optcase = (((struct exprblock *)thisctl->loopvar)->vtype == TYINT);
		...
		if(optcase && ncases>2 && range<2*ncases && range<MAXSWITCH)
			...
			if(cmin < 1)
				excompgoto(q,mknode(TAROP,OPPLUS,
				     cpexpr(thisctl->loopvar), mkint(1-cmin) ));
			else
				excompgoto(q,mknode(TAROP,OPMINUS,
				     cpexpr(thisctl->loopvar), mkint(cmin-1) ));
			}
		else	{
			for(p=thisctl->loopctl ; p!=0 ; p = p->nextcase)
				if(p->labdefined && p->casexpr)
