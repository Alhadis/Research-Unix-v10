...
q1 = p->leftp;
q2 = (q1->tag==TFTNBLOCK ? q1 : ((struct stentry *)q1->sthead)->varp);
if(q2->vtype!=TYUNDEFINED && q2->vtype!=TYSUBR)
	{
	dclerr("attempt to use a variable as a subroutine", ((struct stentry *)p->sthead)->namep);
	frexpr(p);
	return;
	}
...
if(q1->tag == TNAME)
	{
	if( equals(((struct stentry *)q2->sthead)->namep, "stop") )
		{
		exlab(0);
	...
	if( ioop(((struct stentry *)q2->sthead)->namep) )
		{
		exioop(p,NO);
		goto done;
		}
...
if( (q1=p->rightp) && tailor.hollincall)
	for(q1 = q1->leftp ; q1 ; q1 = q1->nextp)
		if( ((struct headbits *)(q2 = q1->datap))->tag==TCONST
		    && q2->vtype==TYCHAR)
			{
...
n = strlen(p);
q = convic(n);
s = t = (char *)calloc(n + 2 + strlen(q) , 1);
while(*q)
	*t++ = *q++;
*t++ = 'h';
while(*t++ = *p++ )
...
if(p1->tag!=TASGNOP || p1->subtype!=OPASGN || ((struct headbits *)p1->leftp)->tag!=TNAME)
	{
