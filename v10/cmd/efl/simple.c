...
case TNOTOP:
	...
	case TCONST:
		if(equals(lp->leftp, ".false."))
			e->leftp = (int *)copys(".true.");
		else if(equals(lp->leftp, ".true."))
			e->leftp = (int *)copys(".false.");
		else goto typerr;
...
case TCALL:
	if( lp->tag!=TFTNBLOCK && ioop(((struct stentry *)lp->sthead)->namep) )
		{
		e = exioop(e, YES);
		exlab(0);
		break;
...
case TASGNOP:
	...
	if(subtype==OP2OR || subtype==OP2AND)
		...
		if(e->vtype == TYCHAR)
			...
			{
			if(((struct typeblock *)lp->vtypep)->strsize != ((struct typeblock *)rp->vtypep)->strsize)
				fatal("simple: attempt to assign incompatible structures");
			e1 = (int *)mkchain(cpexpr(lp),mkchain(rp,
				mkchain(mkint(((struct typeblock *)lp->vtypep)->strsize),CHNULL)));
...
switch(p->tag)
	...
	case TAROP:
		if(p->subtype==OPSTAR && ((struct headbits *)p->rightp)->tag==TCONST
		    && ((struct exprblock *)p->rightp)->vtype==TYINT && vform(p->leftp))
			return(1);
