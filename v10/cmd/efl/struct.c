...
for(p = s->strdesc ; p ; p = p->nextp)
	...
	if(q->vtype==TYFIELD && q->vdim==0 &&
	     (n=conval(((struct fieldspec *)q->vtypep)->frange))*rshift<=fieldmax && rshift>0)
		{
		((struct fieldspec *)prevp->vtypep)->fanymore = 1;
		((struct fieldspec *)q->vtypep)->frshift = mkint( (int) rshift );
		rshift *= n;
		cfree(q->voffset);
...
switch(t = p->vtype)
	...
	case TYSTRUCT:
		*s = ((struct typeblock *)p->vtypep)->strsize;
		*a = ((struct typeblock *)p->vtypep)->stralign;
		*m = ((struct typeblock *)p->vtypep)->basetypes;
		return;
	...
	case TYSUBR:
		...
		*a = 1;
		*m = 1;
		dclerr("subroutine name as variable", ((struct stentry *)p->sthead)->namep);
		return;
