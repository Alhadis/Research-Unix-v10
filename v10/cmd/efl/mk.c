...
for(p = (int *)commonlist ; p ; p = p->nextp)
	if(equals(s, ((struct comentry *)p->datap)->comname))
		return(p->datap);

p = (int *)ALLOC(comentry);
for(t = p->comname ; *t++ = *s++ ; ) ;
p->tag = TCOMMON;
...
switch(t)
	...
	case TAROP:
		...
		if(o==OPPLUS && l->tag==TNEGOP &&
		  (r->tag!=TCONST || ((struct headbits *)l->leftp)->tag==TCONST) )
			{
			p->subtype = OPMINUS;
			p->leftp = r;
			p->rightp = l->leftp;
	...
	case TASGNOP:
		...
		if(lt==TYCHAR || rt==TYCHAR || lt==TYLOG || rt==TYLOG)
			...
			{
			if(lt!=rt || ((struct typeblock *)l->vtypep)->strsize!=((struct typeblock *)r->vtypep)->strsize
				|| ((struct typeblock *)l->vtypep)->stralign!=((struct typeblock *)r->vtypep)->stralign)
				{
...
if(p->tag == TLABEL)
	{
	laberr("attempt to use label as variable", p->namep);
	return( errnode() );
	}
if(instruct || p->varp==0 || ((struct headbits *)p->varp)->blklevel<blklevel)
	{
	q = allexpblock();
	q->tag = TNAME;
	q->sthead = p;
...
if(!instruct)
	{
	if(p->varp && ((struct headbits *)p->varp)->blklevel<blklevel)
		hide(p);
	if(p->varp == 0)
		p->varp = q;
	}
...
if(fn1->tag == TERROR)
	...
	{
	funct = ((struct stentry *)fn1->sthead)->varp;
	frexpblock(fn1);
	}
else
	funct = fn1;
if(funct->vclass!=0 && funct->vclass!=CLARG)
	{
	exprerr("invalid invocation of %s",((struct stentry *)funct->sthead)->namep);
	frexpr(args);
	return( errnode() );
	}
else	extname(funct);
	...
	if( q->vtype == TYSTRUCT)
		...
		for(i = 0; i<NFTNTYPES ; ++i)
			if(q->vbase[i] != 0)
				...
				else	p = p->nextp = (int *)mkchain(r, p->nextp);
				r->vtype = ftnefl[i];
...
*/
/* dirty fixup for wm coughran */
		else	{
			if(blklevel > q->blklevel)
				labwarn("%s is illegally placed\n",p->namep);
...
if(++nxtindif < MAXINDIFS)
	return(nxtindif);
fatal("too many indifs"); return 0;
}
...
if(p = (int *)name(s,1))
	if(p->blklevel == 0)
		...
		if(blklevel > 0)
			...
			if( strcmp(v, ((struct defblock *)(q=p->varp))->valp) )
				{
...
q = (int *)ALLOC(defblock);
p->tag = TDEFINE;
q->tag = TDEFINE;
p->blklevel = q->blklevel = (blklevel==0 ? 0 : 1);
q->sthead = p;
p->varp = q;
((struct defblock *)p->varp)->valp = copys(v);
return(p);
}
...
if(p->vsubs == 0)
	if(p->vdim==0 && p->vtype!=TYCHAR && p->vtype!=TYSTRUCT)
		...
		if(p->vdim)
			{
			s = 0;
			for(q = ((chainp)p->vdim)->datap ; q ; q = q->nextp)
				s = (int *)mkchain( mkint(1), s);
...
if(g = generic(s = ((struct stentry *)((struct typeblock *)p->leftp)->sthead)->namep))
	{
	if(((struct headbits *)p->rightp)->tag==TLIST && ((struct exprblock *)p->rightp)->leftp
		&& ( (vt = typearg(((struct exprblock *)p->rightp)->leftp)) >=0)
		&& (t = g->genftype[vt]) )
		{
...
fp = p->leftp;
setvproc(fp, PROCYES);
fp1 = ((struct stentry *)fp->sthead)->varp;
s = ((struct stentry *)fp->sthead)->namep;
...
if(p->vtype==TYUNDEFINED && fp->vclass!=CLARG)
	if(t = knownfunct(s))
		...
		fp1->vtype = t;
		builtin(t,((struct stentry *)fp1->sthead)->namep);
...
if(p0 == NULL)
	return(-1);
maxt = ((struct exprblock *)p0->datap)->vtype;

for(p = (chainp)p0->nextp ; p ; p = (chainp)p->nextp)
	if( (vt = ((struct exprblock *)p->datap)->vtype) > maxt)
		maxt = vt;

for(p = p0 ; p ; p = (chainp)p->nextp)
...
switch(t->attype)
	...
	case TYCOMPLEX:
		if(e->tag==TLIST)
			if(e->leftp==0 || ((chainp)e->leftp)->nextp==0
			    || ((chainp)((chainp)e->leftp)->nextp)->nextp!=0)
				{
				exprerr("bad conversion to complex", "");
				return( errnode() );
				}
			else	{
				((chainp)e->leftp)->datap = simple(RVAL,
						((chainp)e->leftp)->datap);
				((chainp)((chainp)e->leftp)->nextp)->datap = simple(RVAL,
						((chainp)((chainp)e->leftp)->nextp)->datap);
				if(isconst(((chainp)e->leftp)->datap) &&
				   isconst(((chainp)((chainp)e->leftp)->nextp)->datap) )
					return( compconst(e) );
				e1 = mkcall(builtin(TYCOMPLEX,"cmplx"),
					arg2( coerce(TYREAL,((chainp)e->leftp)->datap),
					coerce(TYREAL,((chainp)((chainp)e->leftp)->nextp)->datap)));
				frchain( &(e->leftp) );
				frexpblock(e);
				return(e1);
				}
...
if(p == 0)
	goto err;
if(((struct exprblock *)p->datap)->vtype == TYLREAL)
	prec = TYLREAL;
