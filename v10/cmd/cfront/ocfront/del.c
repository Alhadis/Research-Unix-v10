/*ident	"@(#)ctrans:src/del.c	1.1.3.8" */
/************************************************************
...
void type::del()
{
	DB( if(Adebug>=1) {
	    ...
	    if(!node::allocated) error('i',"deleting unallocated type node! %t -- id==%ld",this,node::id);
	    node::allocated = 0;
	);
...
// error('d',"\n*** %d::del() -- '%s'\n",this, t_name?t_name->string:"???");
	DB( if(Adebug>=1) {
	    ...
	    if(!node::allocated) error('i',"deleting unallocated table %s! -- id==%ld", t_name?t_name->string:"???",node::id);
	    node::allocated = 0;
	);
