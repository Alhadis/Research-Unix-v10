/*ident	"@(#)ctrans:src/gram.y	1.6.5.35" */
/*************************************************************************
...
%type <p>	external_def fct_dcl fct_def att_fct_def arg_dcl_list 
		...
		moe 
		tag enumtag class_head class_dcl cl_mem_list 
		cl_mem dl decl_list 
...
%type <pn>	ID FDEF inline_fct_def
...
inline_fct_def	:  FDEF { arg_redec($<pn>1); }
		   base_init block
			{
				Finit($1->tp) = $<pn>3;
...
enum_dcl	:  ENUM LC moe_list RC		{ $$ = end_enum(0,$<pn>3); }
		|  ENUM enumtag LC moe_list RC { $$ = end_enum($<pn>2,$<pn>4); }
...
decl		:  decl arg_list
		...
		|  qualified_tname LP MUL ID RP arg_list
			{
		...
		|  qualified_tname LP elist RP
 		...
 		|  qualified_tname LP RP fct_attributes
 			{ /* function with no argument */
				$$ = Ncopy($<pn>1);
 				$<pn>$->n_oper = TNAME;
 				$<pn>$->tp = new fct(0,0,1);
		...
		|  qualified_tname LP MEMPTR decl RP arg_list
...
c_tp		:  TYPE	
		...
		|  qualified_tname	{ $$ = new basetype(TYPE,$<pn>1); }
		|  c_tp TYPE		
			{ 
				if ( DECL_TYPE != -1 ) 
					$$ = $<pb>1->type_adj($<t>2); 
