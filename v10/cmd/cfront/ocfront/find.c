/*ident	"@(#)ctrans:src/find.c	1.1.5.30" */
/******************************************************************
...
int mex;
int access_control;	// lookup is not a real use
...
// error( 'd', "find_name( %s ): nested class %t",s,nstd_cl ); 
							...
							{
								if ( cl->nested_sig == 0 ) {
									error('w',"%s, accessed within nestedC%t, is visible both globally and within enclosingC%t -- using %n (anachronism)", s, cl, nstd_cl, nn );
									ee = 0;
								}
								break;
							}
	...
	if (n) {
		...
		if (n->base == PUBLIC) {
			if (mex && n->n_scope==0) {
				...
				{
					if ( me == dummy_fct )
						error("global scope cannot access%n: %sM",n,n->n_protect?"protected":"private");
...
//error('d',"e %k",e->base);
	...
	if (rootClass == this ) {
			...
			error("%n cannot access %s: %s is a privateBC",pM->mef,pM->nm,pM->bc->bclass->string);
        		delete pM; pM=0;
    		}
    		rootClass = 0;
    		pubVClass = 0;
		...
		if ( bc->ppp==PRIVATE ) {	// private base
			...
			if ( bc->base == VIRTUAL &&
				...
				if ( pubVClass == 0 ||
					...
					pM->bc = bc; pM->mef = me; pM->nm = s;
				}
			}
			else {
				if ( me == dummy_fct )
					error("global scope cannot access %s: %s is a privateBC",s,bc->bclass->string);
...
// error('d',"check_visibility(%n, %t, %d, %n)",n,cl,tbl,fn);
	...
	Pname nn = new name;
	char* s = Pchar(n->n_tbl_list);	// overloaded name
	...
	for (Plist gl=Pgen(nx->tp)->fct_list; gl; gl=gl->l) {
		...
		if (n == nn) {
			...
			if ( fn == dummy_fct )
				error("global scope cannot access%n: %sM",nn,nn->n_protect?"protected":"private");
