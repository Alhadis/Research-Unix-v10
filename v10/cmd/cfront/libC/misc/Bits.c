...
Bits::operator<<= (int k)
{
	...
	int chunkoffset = k >> Bits_shift;
	if (chunkoffset) {
		register Bits_chunk* dest = limit();
		register const Bits_chunk* src = dest - chunkoffset;
	...
	register int bitoffset = k & Bits_mask;
	if (bitoffset) {
		register Bits_chunk* p = limit();
		register int compoffset = Bits_len - bitoffset;
