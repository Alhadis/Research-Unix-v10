...
public: /* Some enums are declared in ios to avoid pollution of
	 * global namespace
	 */
	enum io_state	{ goodbit=0, eofbit=1, failbit=2, badbit=4, hardfail=200};
...
class streambuf {
	...
	virtual int	sync();
	virtual streampos
			seekoff(streamoff,seek_dir,open_mode =ios::in|ios::out);
	virtual streampos
			seekpos(streampos, open_mode =ios::in|ios::out) ;
...
public:	
	...
	ostream&	operator<<(unsigned char c) { return put(c) ; }
