/*ident	"@(#)ctrans:src/gram.y	1.6.5.21" */
/*************************************************************************
...
%type <p>	external_def fct_dcl fct_def att_fct_def arg_dcl_list 
		...
		moe 
		tag ttag class_head class_dcl cl_mem_list 
		cl_mem dl decl_list 
...
%left	EMPTY
%left	NO_ID
%left	RC LC ID BREAK CONTINUE RETURN GOTO DELETE DO IF WHILE FOR
	CASE DEFAULT AGGR ENUM TYPE TNAME 
%left	NO_EXPR
...
identifier       : ID | TNAME { $<pn>$ = Ncopy($<pn>1) ;} ;
...
/* prevent la_decl from mistaking casts for possible declarations */

base_init_colon : COLON { must_be_expr = 1; }
		
base_init	:  base_init_colon init_list
			{	
				$$ = $2; 
				must_be_expr = 0;
			}
...
tname           : TNAME { $<pn>$ = templp->check_tname($<pn>1) ; }
                | TNAME lt  temp_inst_parms gt
                  { 
...
temp_inst_parm  : new_type
                  {$<pn>1->n_template_arg = name::template_actual_arg_dummy ;
...
enum_dcl	:  ENUM LC moe_list RC		{ $$ = end_enum(0,$<pn>3); }
		...
		|  ENUM tag			{ $<pb>$ = (Pbase)$<pn>2->tp;}
		|  ENUM DECL_MARKER		{ $<pb>$ = (Pbase)$<pn>2->tp;}
		;
...
class_dcl	:  class_head cl_mem_list RC
		...
		|  class_head cl_mem_list RC TYPE
			...
			{ 
                ...
                | AGGR  TNAME lt temp_inst_parms gt
                  { 
		    Pname p = parametrized_typename($<pn>2,
						  (expr_unlist($<el>4))) ;
                    $<pb>$ = (Pbase)p->tp; 
                    check_tag(); }                   
...
/* A variation on the tag non-terminal that also permits parametrized type */
/* names */
ttag         : ID    {  $$ = $1 ; } | tname ;
...
tn_list		:  tscope	{ $<pn>$ = $<pn>1; }
		|  tn_list tscope	{	error("CNs do not nest"); }
...
decl		:  decl arg_list
 		...
 		|  decl LP RP TYPE
 			/* const function with no argument */
 			{	$<pn>1->tp = new fct($<pn>1->tp,0,1);
 				Pfct($<pn>1->tp)->f_const = 1;
 			}
		|  tname arg_list
			{	Pname n = $<pn>1;
				$$ = Ncopy(n);
...
block		:  LC
		...
		|  LC RC
			{	$$ = new block($<l>1,0,0); NOT_EXPECT_ID();}
		|  LC error RC
			{	$$ = new block($<l>1,0,0); NOT_EXPECT_ID();}
		;
...
statement	:  simple SM
		...
		|  SM
			...
			{/* $1 is the formal parameter name */
			 $$ = new estmt(SM_PARAM,curloc,$<pe>1,0);	}
		|  RETURN e SM
...
term_elist	: TYPE LP elist RP
		...
		|  tname LP elist RP
			{	$$ = new texpr(VALUE,$<pn>1->tp,$<pe>3); }
                |  NEW LP elist RP cast_type    { goto new2; }
                |  NEW LP elist RP new_type     /* allow separate allocation */
			{	new2:
				Ptype t = $<pn>5->tp;
				$$=new texpr(NEW,t,0);
				$<pe>$->e2 = $<pe>3;
			}
		|  MEM NEW LP elist RP cast_type	{ goto new4; }
		|  MEM NEW LP elist RP new_type	/* allow separate allocation */
			{	new4:
				Ptype t = $<pn>6->tp;
				$$ = new texpr(GNEW,t,0);
				$<pe>$->e2 = $<pe>4;
...
prim		:  ID
		...
		|  tscope OPERATOR c_type
			{	$$ = $3;
				sig_name($<pn>$);
				$<pn>$->n_qualifier = $<pn>1;
			}
		|  tscope COMPL TNAME	/* allow explicit call of destructor */
			{	
...
arg_list	:  arg_lp arg_type_list ellipsis_opt RP
		    { 
			$$ = new fct(0,name_unlist($<nl>2),$<t>3); 
			in_arg_list=0;
		    }
		|  arg_lp arg_type_list ellipsis_opt RP TYPE
		    { 
			$$ = new fct(0,name_unlist($<nl>2),$<t>3); 
			in_arg_list=0;
			if ($<t>5 != CONST)
			if ( la_look() != SM ) {
				error("syntax error: unexpected%k (did you forget a `;'?)",$<t>5);
                        	{ YYSTYPE y; y.t = SM; la_backup(SM,y); }
                        	lex_unget($5);
 			}
				else error("syntax error: unexpected%k",$<t>5);
			Pfct($<pt>$)->f_const = 1;
		    }
		;
