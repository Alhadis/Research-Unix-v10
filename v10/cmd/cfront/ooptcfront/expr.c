...
Ptype add_const_to_type(Ptype t, int cnst)
{
    switch(t->base) {
      case RPTR:
	Pptr(t)->rdo = cnst;
...
*/
{
	//if (this == 0) error('i',"0->expr::typ");
        if(permanent == 3 && catch_delete_bugs) error ('i', "expr::typ of a deleted expr.");
...
//error('d',"expr %d %k %n %t",this,b,b==NAME?this:0,tp);
...
extern Pname Ntmp;
	...
	case CCON:
		tp = c_strlen(string)>1 ? char_type : int_type;	// stored as 'a'
		goto save;
...
// error( 'd', "tn: %n %k %t", tn, tn->tp->base, tn->tp );
...
//error('d',"e %k %t %n",e->base,e->tp,e->base==NAME?e:e->base==REF?e->mem:0);	
	    ...
	    int e1_const = e1 && e1->tp->tconst() && !vec_const && !fct_const;
	    Pptr e1_ptr;
	    if (e1 && e1->tp) e1_ptr = e1->tp->is_ptr ();
	    /* const foo * C; C->bar; is a const */
	    int e1_pt_const = e1_ptr && e1_ptr->typ->tconst () && !vec_const
		&& !fct_const;

	    if((e1_const && base == DOT) || e1_pt_const)
		result->tp = add_const_to_type(result->tp, 1);
...
//error('d',"mm %t %t %d %d",mm,pm,pm->baseof(mm),mm->has_base(pm));
	...
	case DEREF:
		...
		//	t->vec_type();
			if (t->base==PTR && Pptr(t)->memof) error("P toM deRd");
			tp = t->deref();
		}
...
//error('d',"deref %t[%t]",t1,t2);
			if (t1->vec_type()) {		// e1[e2]
				...
				if (t1->base==PTR && Pptr(t1)->memof) error("P toM deRd");
				tp = t1->deref();
				(void) tp->tsizeof();
			}
				...
				if (t2->base==PTR && Pptr(t2)->memof) error("P toM deRd");
				tp = t2->deref();
				(void) tp->tsizeof();
			}
			else {
...
//error('d',"addrof(%d) %k %d",base,e2->base,e2->base);
	...
	//	tp = t->addrof();
		if (t->base==FCT) Pptr(tp)->memof = Pfct(t)->memof;
...
//error('d',"n2 %k t %t cn %n",n2->base,t,cn);
			if (cn == 0) break;
...
// error( 'd', "e2->mem->base: %k", e2->mem->base );
			...
			if (f->base == FCT) {	// &p->f
				...
				bound = 1;
				error('w',"address of boundF (try using ``%s::*'' forPT and ``&%s::%s'' for address)",m->n_table->t_name->string,m->n_table->t_name->string,m->string);
...
extern nin;
		...
		case INCR:
		case DECR:	// ++a=b => ++a,a=b
 		{       // Skip transformation if it is a post incr or decr
 			if (!e1->e2) break ;
extern nin;
			base = G_CM;
			nin++;
...
*/
//error('d',"c1 %n %d c2 %n %d",c1,c1?c1->tp:0,c2,c2?c2->tp:0);
					if (c2
					&& c2->tp
					&& can_coerce(t1,t2)==0
...
/* ODI notes -
   ...
   propagate const up the expr tree to enforce it correctly 
