/*ident	"@(#)ctrans:src/simpl.c	1.5.4.25" */
/******************************************************************
...
extern Pexpr initialize_vptrs (Pclass cl, Pexpr this_expr);

    /* Constructs an expression which initializes the given class's vptrs.
       The this_expr forms the first part of the path expression for each
       vptr assignment. The vptr assignments are combined into a comma
       expression. */
	...
	{	Pfct f = Pfct(tp);
		...
		if (th) {
			...
			&& th->n_assigned_to!=FUDGE111)
				error('w',&where,"assignment to this");
		}


		if (ansi_opt && f->body==0) {
		// make sure arguments can be printed for decls
			
			for (Pname n = f->f_args; n; n = n->n_list) {
				n->n_scope = ARG;
				if (n->string) {
					char* ps = new char[strlen(n->string)+1];
					strcpy(ps,n->string);
					n->string = ps;
				}
/*
				if (n_oper==DTOR && n->n_list==0) {
					// add __free argument
					Pname fa=new name;
					fa->tp = int_type;
					n->n_scope = ARG;
...
/* But this is not needed on a sun, and just makes the debugger print trash */
#ifndef sun
					if (t && t==void_type && ansi_opt == 0)
...
//error('d',"simpl base %t i %d ",l->bclass,i);
	...
	// initialize the vptrs that are updated by this class
        ...
        if (th) {
	    Pexpr ee = initialize_vptrs (cl, th);
	    if (ee)
		init_list = init_list ? new expr(CM,init_list,ee) : ee;
	  }

        else {
            error('i', "%t->fct::ctor_simpl(): th found to be null", cl);
	  }
...
extern char *make_local_name(Pclass, int=0);
                ...
                Pexpr vtbl = new text_expr(blist->string,str?str:cl->string);
...
Pexpr initialize_vptrs (Pclass cl, Pexpr this_expr)
...

{
	...
	topclass = cl;
	Pexpr vptr_exp = cl->get_vptr_exp(blist->string);
	...
	Pexpr ee;
	if (this_expr)
	    ee = new ref(REF, this_expr, vptr_exp);
	else
	    ee = vptr_exp;
