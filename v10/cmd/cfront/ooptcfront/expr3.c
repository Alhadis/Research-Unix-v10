/*ident	"@(#)ctrans:src/expr3.c	1.3.5.31" */
/***************************************************************************
...
static exact1(Pname,Ptype);
static exact2(Pname,Ptype);
static exact3(Pname,Ptype);

static Pname user_dominate(Pname n1, Pname n2)
{
	Pfct f1 = Pfct(n1->tp);
	Pfct f2 = Pfct(n2->tp);
...
//error('d',"user_dominate: %n %t %t",n1,f1,f2);
	...
	if (k1 && k2==0) return n1;
	if (k2 && k1==0) return n2;
...
//error('d',"t1 %t t2 %t at %t",t1,t2,at);
		if (t1==t2 || t1->check(t2,0)==0) continue;
		Pptr r1 = t1->is_ref();
		Pptr r2 = t2->is_ref();
...
//error('d',"pr3 %d",r);
		...
		else if (1<level) {	// try standard conversions
			if (exact3(a1,at))
				rr = n1;
			if (exact3(a2,at)) {
				if (rr) return 0;
...
//error('d',"exact3 %d",nt->check(at,ASSIGN));
	return nt->check(at,ASSIGN)==0;
}
...
//error('d',"std_match(%n) %t",n,n->tp);
	...
	for (e=arg, nn=f->argtype; e; e=e->e2, nn=nn->n_list) {
		...
		if (at->base == ANY) return 0;
		if (nn == 0) return 0;//f->nargs_known==ELLIPSIS;
...
tptp:
	...
	if (c1) {
		...
		if (c2 && c2->tp==cl) return 1;
 		...
 		if (c2 && (Pclass(c1->tp)->same_class(Pclass(c2->tp)))) return 1 ;
...
//error('d',"tt %t t2 %t -> %d cp %d",tt,t2,tt->check(t2,COERCE),const_problem);
...
extern Pname conv_dominates(Pname,Pname);
		for (Pname ox, on=cl->conv; on; on=ox) {
			...
			ox = on->n_list;
			Nstd = 0;
//error('d',"on %n -> %d",on,t1->check(f->returns,COERCE));
...
//error('d',"name %n %k",fn,fn->n_oper);
	...
	case DOT:
		...
		e1->n_initializer = 0;
	//	const_obj = (e1&&e1->e1)?e1->e1->tp->tconst():0;
		if (e1 && e1->e1) {
			Ptype t = e1->e1->tp;
			Pptr tt = t->is_ptr_or_ref();
//error('d',"t %t tt %t",t,tt);
			const_obj = (tt ? tt->typ : t)->tconst();
		}
	case MDOT:
	{	Pexpr n = e1->mem;
	lxlx:
...
//error('d',"user2 %n %t",nn,nn->tp);
				if (found) {
					...
					if (d == 0) d = user_dominate(found,nn);
					if (d == 0) {
						error("ambiguousA for%n: %t and %t",fn,found->tp,nn->tp);
						goto fnd;
					}
					else
						found = d;
...
//error('d',"c1 %n",c1);
	if (c1) {	// assigning to a const X & is fine
		...
		Ptype p1b = p1;
		while (p1b->base==TYPE) p1b = Pbase(p1b)->b_name->tp;
		int bc = Pbase(p1b)->b_const;
		Pbase(p1b)->b_const = 0;
		refd = 1;	/* disable itor */
		Pexpr a = class_init(0,p1,init,tbl);
		Pbase(p1b)->b_const = bc;
		refd = 0;
...
//error('d',"ri a %d %k",a->base,a->base);
	...
	//	case G_CM:
		...
		a = a->address();
//error('d',"ri2 a %d %k",a->base,a->base);
		a =  ptr_init(px,a,tbl);
//error('d',"ri3 a %d %k",a->base,a->base);
		return a;
//		return ptr_init(px,a->address(),tbl);//a->address();
	}

//error('d',"p1 %t it %t",p1,it);
/* NOTE: in this region, several uses of "it" were left even though
   the resolution of it to its basetype was removed above.
   Since these are type checks, it seems a a bad idea to ignore signed/unsigned
   and related items. --benson */
	if (p1->check(it,0)) {
		...
		//		goto xxx;
			...
			//	int i; double& r = i;
			if (init->lval(0) && p1->tconst()==0) error('w',"conversion of lvalue needed: temporary used toI reference");
			goto def;
		}
...
/* ODI notes
   ...
   fixes for template classes and to get const right
   by not discarding const information from typedefs 
