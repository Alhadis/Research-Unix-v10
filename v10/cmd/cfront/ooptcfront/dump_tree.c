/* -*- Mode:C++ -*- */
/* dump_tree.C -- utilities for displaying the cfront tree. */
/* BIM 890530 */
/* rewritten 890712 to use the tree-walking facilities */

/*
	$Source: /usr3/lang/benson/work/stripped_cfront/RCS/dump_tree.c,v $ $RCSfile: dump_tree.c,v $
...
#include "dump_tree.H"
#include <stdio.h>		/* turds and whey */
#include <malloc.h>
#include "template.h"
...
displayer::a_node() 
{
    ...
    nz_printf("$node", (n.n_key || n.permanent)) ;
...
displayer::a_name()
{
    ...
    *arg->output_stream << "\n";
	...
	switch(n.n_template_arg) 
		...
		break;
	    case name::template_actual_arg_dummy:
    ...
    if(n.output_string) {
	print_string("output_string", (void *)n.output_string);
    }
...
/* Note -- declared extern "C" in dump_tree.H. */
...
static char rcsinfo[] = "$Header: /usr3/lang/benson/work/stripped_cfront/RCS/dump_tree.c,v 1.1 89/11/20 08:50:22 benson Exp $";


/* $Log:	dump_tree.c,v $
 * Revision 1.1  89/11/20  08:50:22  benson
 * Initial revision
 * 
Revision 1.32  89/11/03  16:18:32  benson
mind rot.

Revision 1.31  89/11/03  16:10:04  benson
Oh, don't try to use the name constructor inside dgb.

Revision 1.30  89/11/03  15:28:39  benson
fix cn for tables.

Revision 1.29  89/10/31  14:47:24  benson
new function flags. Bug in vector printing.

Revision 1.28  89/10/20  14:33:06  benson
print abnormal function flag, don't print n_list and s_list peers
of top item dumped.

Revision 1.27  89/10/06  14:44:28  benson
use new token name array courtesy of byacc.

Revision 1.26  89/10/04  18:54:21  dysak
Add code to dump the let_address_of_deref_be_address_of and
   nonempty_converted_collection flags in an expression node.
Add code to dump the n_persistent_db field of a name node.
Fix failure to discriminate properly between the n_realscope
   and n_qualifier nodes properly when dumping.

Revision 1.25  89/09/22  14:03:32  benson
fixes for locs.

Revision 1.24  89/09/14  15:25:47  benson
display a table as the top level item, but not deeper in.

Revision 1.23  89/09/13  09:17:10  benson
rename aggregate to collection.

Revision 1.22  89/09/08  13:49:52  benson
fix handling for dt_normal to print something,
and also stop printing a spurious >.

Revision 1.21  89/09/02  22:08:10  benson
deal with changes to tree copies.

Revision 1.20  89/08/29  13:39:57  benson
disentangle the character string name of a generic function
from n_table_list.

Revision 1.19  89/08/24  08:45:11  benson
some support for queries and templates.

Revision 1.17  89/07/27  13:30:33  benson
agg changes.

Revision 1.16  89/07/21  16:49:44  benson
checked in with -k by benson at 89.07.24.15.48.50.

Revision 1.16  89/07/21  16:49:44  benson
support for by_name nodes.

Revision 1.15  89/07/18  15:17:39  benson
print local classes, not args, under label local_classes.
other bugfixes.

Revision 1.14  89/07/14  10:58:30  benson
rewrite as a user of the new tree_walk facilities. Brief mode
is most likely busted.

Revision 1.13  89/07/10  10:51:32  benson
add fields added in 2.0 or just plain missed before.

Revision 1.13  89/07/09  14:37:21  benson
print aggregate facts. Treat QUERIES as exprs. 

Revision 1.12  89/06/30  15:28:26  benson
checked in with -k by benson at 89.07.07.08.32.34.

Revision 1.12  89/06/30  15:28:26  benson
add ZTYPE to the list of type nodes, whatever it is.

Revision 1.11  89/06/30  14:34:07  benson
run script to make token name list automatically from tokens
in gram.y

Revision 1.9  89/06/29  08:44:47  benson
checked in with -k by benson at 89.06.29.20.54.47.

Revision 1.9  89/06/29  08:44:47  benson
token_name function useful in parse tracing.

Revision 1.8  89/06/28  12:57:53  benson
