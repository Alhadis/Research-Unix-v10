/*ident	"@(#)ctrans:src/lex.c	1.3.4.17" */
/***************************************************************************
...
char* txtmax = &inbuf[FIRSTCHUNK/*TBUFSZ*/-1];
char* txtstart = 0;
char* txtfree = 0;
...
int linkage;				// linkage is default C++	
					// linkage==0	=> C++ linkage
					// linkage==1	=> C linkage
const LINKMAX = 10;
static lvec[LINKMAX];
...
void pragma_set_linkage (char *p)
{
    if (bl_level) error("linkage pragma inside block");
    set_linkage(p);
}
...
#define reti(a,b)	{ addtok(a, rt(b));          return a; }
#define retn(a,b)	{ addtok(a, rt((Pnode)b));   return a; }
#define rets(a,b)	{ addtok(a, rt(b));          return a; }
#define retl(a)		{ addtok(a, rt(curloc));     return a; }
...
*/
{
/* The ktbl is only for types. We put nothing in it. */
	ktbl = new table(KTBLSIZE,0,0);

/* The keyword_table is for the reserved words */
	keyword_table = new table(67,0,0);
...
void lex_init()
{
	...
	file_name[0] = src_file_name;
//	curloc.file = 0;	// spurious: curloc is a static
	curloc.line = 1;
...
//	Ntoken++;
	...
	for(;;) {
		...
		if (saved) {
 		...
 		if (lxchar+1 >= CSSZ )
		 error( "illegal input character enountered: %d", lxchar );
		...
		case A_EOF:
			if (br_level || bl_level+lcount)
				error("'%s' missing at end of input",(bl_level+lcount) ? "}" : ")");
		...
		case A_DOLL:
			{ // lex a name of the for $id for template tree formals
			  ...
			  // TBD: make sure that the pragma is set
			  ...
			  error("%s wasn't a statement or expression formal",
				txtstart) ;
...
//error( 'd', "lex: bl_level: %d txtstart %s", bl_level, txtstart);
// local class
...
//('d',"lex tname %n",n);
				if (bl_level > 1) {
					Pname nn = ktbl->look(txtstart,LOCAL);
					if ( nn ) {
						n = nn;
//error( 'd', "lex: local class instance: %n", nn );
					}
				}
					retn(TNAME,n);
				case LOC:
...
// local class
			if ( bl_level && 
			     (n=ktbl->look(txtstart,LOCAL)) )
{
//error( 'd', "lex2: local class instance: %n", n );
					retn(TNAME,n);
}
			else
			  rets(ID, copy_if_need_be(txtstart)) ;
		...
		getfp2:
			...
			case 'u':
			case 'U':
				if (ret==FCON) error("%c suffix for floating constant",lxchar);
			case 'l':
			case 'L':
				txtfree--;
				pch(lxchar);
				switch (get(lxchar)) {	// ul, Lu, ets.
				case 'l':
				case 'L':
				case 'u':
				case 'U':
...
//&& dead, dead, dead			goto push;
					...
					else if ( (fn=file_name[file_stack[curr_file]])
...
//						Nstr++;
		...
		default:	// detect #pragma
				// pass #rubbish through
		{	char* p = txtstart+1;
			curloc.line = cl;
			pch('#');
			pch(c);
...
/* ODI notes -
   ...
   separate ktbl from keywords to because it dosen't contain
   legitimate nodes.
*/
