/*ident	"@(#)ctrans:src/dcl4.c	1.3.4.33" */
/**************************************************************************
...
vhack = 1;
	...
	if (hit) error('w',&n->where,"%n hides virtual%n ofT %t",n,vn,vn->tp);
...
//error( 'd', "out of loop: vc: %d vpsz: %d", vc, vpsz );
		...
		for (Pname nn=memtbl->get_mem(i=1); nn; nn=memtbl->get_mem(++i) ) {
			Pfct f = Pfct(nn->tp);
                ...
                if ( vc >= vpsz ) { // resize vp vector
                        ...
                        delete [vpsz] vp;
                        vp = tvp;
//error( 'd',"resizing2: vc: %d vpsz: %d tvpsz: %d", vc, vpsz, tvpsz );
                        vpsz = tvpsz;
                }
...
{
//error('d',"%t::all_virt(%t,%s,%d)",this,bcl,s,leftmost);
	int i = 0;
...
/*
	int mod = 0;	// first determine argument type
{
	for (Pbcl b = baselist; b; b = b->next) {
		Pclass bcl = b->bclass;
		switch (b->base) {
...
if (def) {
	...
	if (warning_opt && 128<obj_size)
		error('w',"copying a %d byte object (ofC%n)",obj_size,this);
...
//error('d',"base %t %k offset %d ptroffset %d",bcl,b->base,b->obj_offset,b->ptr_offset);
	...
	/*AAA			// can copy vptr, X(X&) sets it anyway
...
//	cl->c_xref |= 4;  // now it has X::operator=(const X&)
	...
	if (warning_opt && 128<cl->obj_size)
		error('w',"copying a %d byte object (ofC%n)",cl->obj_size,cl);
{
	int slow = 0;	// slow==0 => copy using vector copy
...
//error('d',"base %t",bcl);
		...
		case VIRTUAL:
			if (warning_opt)
				error('w',"copying an object ofC%n with s virtualBC",cl);
			if (b->ptr_offset) {
...
//error('d',"classdef %s tbl %d gtbl %d",string,tbl,gtbl);
...
// local classes
//error('d', "classdef(): lex_level: %d nof: %n", lex_level, cc->nof);
	if (lex_level) {
    		in_fct = cc->nof;
    		// local classes at lex_level==1 
...
//error('d',"base %t %k init %d",cl,l->ppp,l->init);
	...
	if (cl == baselist->bclass) {	// pad to ensure alignment:
		boff = cl->real_size;
		int xtra = boff%ba;	// not obj_size-real_size, we can
		...
		if (xtra) boff += ba-xtra;	// align
		l->obj_offset = boff;
		boff += cl->obj_size;	// don't use waste
	}
				bvirt += cl->virt_count;
			}
...
//	int nmem = mem_list->no_of_names();
	...
	bt->b_table = memtbl;
	Ptype cct = bt->addrof();
	if (strict_opt) Pptr(cct)->rdo = 1;	// type of `this': X *const
	...
	*/
		...
		case PROTECTED:
			...
			protect = PROTECTED;
		prpr:
			if (in_union) error("%k in unionD",p->base);
			continue;
...
//error('d',"mem%n %d scope %d n_sto %k",p,p->tp->base,scope,p->n_sto);
		if (scope==0) st = 0;
...
//error('d',"inline %d",f->f_inline);
		...
		//	case STATIC:
			...
			if (m == 0) continue;
//error('d',"p %n %t %d",m,f,m->n_initializer);
			if (m->n_initializer) c_abstract = 1;
//error('d',"mem %n: %t ff %d protect %k",m,m->tp,ff,protect);
			if (ff == 0) m->n_protect = protect;
			if (b) {
//error('d',"b line %d %d defined %d",p->where.line,m->where.line,m->tp->defined);
			...
			if (ff==0 && p->where.line!=m->where.line)
				error("%n cannot be redeclared inCD",p);
		}
		else {
			Eppp = scope?scope:protect?protect:0;

			if (p->base == TNAME) {	// typedef names are exported
						// to surrounding scope
						// NOT complete: should go to
			...
			// typedefs need to be generated outside class
					...
					if ( n && lex_level == 0 && n->base != TNAME )
                                                error("%nredefined: identifier and typedef",p); 
                                p->dcl_print(0);
                                }
                                continue;
                        };
...
// error('d',"lex_level: %d p %n lex %d",lex_level, p,p->lex_level);
...
extern Plist stat_mem_list;
	...
	if (dt && ct==0) error('w',"%s has%n but noK",string,dt);
...
/* ODI notes -
   ...
   virt's changed to avoid bad union pun.
*/
