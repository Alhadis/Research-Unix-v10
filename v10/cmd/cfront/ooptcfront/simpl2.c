...
Pexpr cdvec(Pname f, Pexpr vec, Pclass cl, Pname cd, int tail, Pexpr i)
/*
...
// local static class object
// error('d', "block::simpl: n %n in %t", n, in );
			if (in && in->base==STAT_INIT) {
					// dynamic initialization
					// introduce first time switch
				Pname cn;
				Pname x;
				Ptype ct;
				if ( cn = n->tp->is_cl_obj() ) {
// error( 'd', "block::simpl: cl_obj_vec: %t", cl_obj_vec );
    					ct = new ptr(PTR,n->tp);
				...
				if ( cn ) {
					...
					if ( dtor ) {
						Pexpr eee = new expr(DEREF, x, 0 );
    						...
    						c->tp = any_type;
    						Pexpr cc = new expr( QUEST, c, zero );
    						cc->cond = x;
					...
					Pexpr xe = new expr( G_ADDROF, 0, n );
    		...
    		if ( in ) {
	 		if ((in->base == ILIST && in->e2 == 0) ||
             		    (in->base == STRING && n->tp->base == VEC))
                  		if (ansi_opt==0) {
                      			error('s',&n->where,"initialization of automatic aggregate");
                      			continue;
                   		}
    		}    
		...
		if ( cln=n->tp->is_cl_obj() ) {
			...
			if (d) {			// n->cl.dtor(0);
// local static class object
// suppress dtor for local static class object
				if ( n->n_stclass == STATIC 
					&& in 
					&& in->base==STAT_INIT )  
						goto stat_init;
...
// error('d',"vec %n: in %k c %n",n,in?in->base:0,c);
			n->n_initializer = 0;
...
void expr::simpl()
{
//error('d',"%d->expr::simpl %k e1=%d e2=%d",this,base,e1,e2);
...
//error('d',"simpl & %k",e2->base);
	...
	{	Pname n = 0;
		...
		if (n) error('s',"call of inline void%n in commaE",n);
		// no break
	}
	case NAME:
	case DUMMY:
...
//error('d',"uninline %n %d %d",fn,f->body,f->f_expr);
	...
	if (warning_opt) error('w',"%n too complex for inlining",fn);
...
//error('d',"use new %d used %d init %d",base,new_used,init);
...
extern nin;
				nin = 1;
				if (r->not_simple()) error('s',"PE too complicated for delete");	
				nin = 0;
#ifndef sun
...
/* ODI notes
   ...
   set Cstmt to get better error messages
  
   on sun, don't convert void to char *

   ditch support for RETBUG, it made a union hard to walk in the tree.
*/
