...
/* ok, name stuff */
	...
	if(n.n->output_string) {
	    n.n->output_string = tci->malloc(strlen(n.n->output_string)+1);
	    strcpy(n.n->output_string, Pname(node)->output_string);
	}
...
static char rcsinfo[] = "$Header: /usr3/lang/benson/work/stripped_cfront/RCS/tree_copy.c,v 1.1 89/11/20 08:51:04 benson Exp $";


/*
   $Log:	tree_copy.c,v $
 * Revision 1.1  89/11/20  08:51:04  benson
 * Initial revision
 * 
 * Revision 1.5  89/10/10  08:43:12  benson
 * new error system prototype.
 * 
 * 
 * Revision 1.4  89/09/02  22:06:38  benson
 * add ability to keep something out of the hash table.
 * 
 * Revision 1.3  89/08/24  10:01:06  benson
 * additional argument as a walker action procedure.
 * 
 * Revision 1.2  89/08/11  15:07:19  sam
 * - stopped copying at "defined" types
 * - stop copying ofthe pre-defined exprs zero, and dummy
 * - if the hook returns a new node, use it rather than mallocing a new one.
 * 
 * Revision 1.1  89/07/21  16:46:03  benson
 * Initial revision
 * 
