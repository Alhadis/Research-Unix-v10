...
// error('d', &T->place, "use_token: %s", T->retval.s );
		if (last_tokens[one_back]==COLON &&
			last_tokens[two_back]==PR &&
		...
		if (last_tokens[one_back]==LC &&
			last_tokens[two_back]==RP &&
			(n=ktbl->look(T->retval.s,NESTED))) {
				T->tok=TNAME;
				T->retval.pn=n;
				break;
...
// error('d', "ccl: %t fd: %d, in_mem_fct: %d, in_class_decl: %d", ccl, fd, in_mem_fct, in_class_decl);
			...
			(bl_level == ccl->lex_level + in_class_decl + 1))
				...
				if ( n->lex_level &&
					...
					n = n->tname( lastval.t );
					modified_tn = modified_tn->l;
					if (gtbl->look(n->string,0)) statStat = n;
				}
			}
			else {
		...
		if ( tk == ID ) 
		{
			...
			if (curr_scope||ccl||nstd) {
				...
				if (nstd && must_be_id == 0 && (in_expr == 0 || (lasttk==LP && tk2==RP))) {
