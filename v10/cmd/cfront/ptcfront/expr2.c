...
extern int pr_dominate(Ptype, Ptype);
static Pname dominate(Pname,Pname,Pexpr,int);
...
//error('d',"mt1 %t mt2 %t",mt1,mt2);
		...
		else if (mo < STD) {	// user-defined conversion user
...
//error('d',"glob %d",go);
	...
	if (go) {
		if (go < STD) {	// user-defined conversion necessary => binary
			if (try_to_demote(bb,t1,t2))
...
Pname dominate(Pname n1, Pname n2, Pexpr arg, int const_obj)
/*
	the two functions n1 and n2 are equal after the intersect
        rule is applied.  Does the one dominate the other in terms
        of const?
...
//error('d',"const_problem %t %t %t %d",t1,t2,at,const_problem);
	...
	nc:
		...
		if (t1==at || exact1(a1,at)) {
			rr = n1;
			if(t2==at || exact1(a2,at))
				continue;
		}
		else if (t2==at || exact1(a2,at))
			rr = n2;
		else {	// try integral promotion
			if (exact2(a1,at))
				rr = n1;
			if (exact2(a2,at)) {
				if (rr) rr = 0;
		...
		if (!rr) {	// try standard conversions
			if (exact3(a1,at))
				rr = n1;
			if (exact3(a2,at)) {
				if (rr) rr = 0;
