...
*/
		...
		default:
			if (c == ','){
				...
				if(isdigit(c)){
					putch(c,NO);
					while(isdigit(c=getc(input))||c == '.')putch(c,NO);
					if(c == 'e'){
						putch(c,NO);
						c = getc(input);
						putch(c, NO);
						while(isdigit(c=getc(input)))putch(c,NO);
					}
					ungetc(c,input);
				}
				else putch(c,NO);
...
gotif(void){
	...
	if(++clev->iflev >= IFLEVEL-1){
		fprintf(stderr,"too many levels of if %d\n",clev->iflev );
...
gotdo(void){
	if(++dolevel >= DOLEVEL-1){
		fprintf(stderr,"too many levels of do %d\n",dolevel);
		dolevel = DOLEVEL-1;
	}
...
error(char *s)
{
	fprintf(stderr,"saw EOF while looking for %s\n",s);
	exit(1);
}
