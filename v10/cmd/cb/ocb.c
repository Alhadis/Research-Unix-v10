	...
	while ((c = getch()) != EOF){
		...
		case '{':
			...
			if(++clev >= &ind[CLEVEL-1]){
				fprintf(stderr,"cb: too many levels of curly brackets at line %d\n", lineno);
				clev = &ind[CLEVEL-1];
			}
			clev->pdepth = 0;
...
gotif(){
	...
	if(++clev->iflev >= IFLEVEL-1){
		fprintf(stderr,"cb: too many levels of if at line %d\n", lineno);
...
gotdo(){
	if(++dolevel >= DOLEVEL-1){
		fprintf(stderr,"cb: too many levels of do at line %d\n", lineno);
		dolevel = DOLEVEL-1;
	}
...
char c;
{
	if(tp < &temp[TEMP-120])
		*tp++ = c;
		...
		if(keep){
			if(tp >= &temp[TEMP-1]){
				fprintf(stderr,"cb: can't look past huge comment - quiting at line %d\n", lineno);
				exit(1);
			}
			*tp++ = c;
		}
			...
			err++;
			fprintf(stderr,"cb: truncating long comment at line %d\n", lineno);
		}
	}
	return(c);
}
