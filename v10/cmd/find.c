...
#define MINUSINF (int)~((unsigned)-1>>1)
...
{
	int or(), and(), errchk();
	struct anode *exlist, *errchklist;
	int paths;
...
usage:		fprintf(stderr, "Usage: find path-list predicate-list\n");
	...
	errchklist = mk(errchk, (struct anode*)0, (struct anode*)0);
	exlist = mk(or, mk(and, exlist, errchklist), errchklist);
	if(Ai<argc) {
...
struct anode *e3() { /* parse parens and predicates */
	int exeq(), ok(), glob(), ctime(), mtime(), atime(), user(),
		...
		type(), ino(), cpio(), newer(), status(), errchk();
	struct anode *p1;
	int i;
	register char *a, *b, s;
	...
	if(EQ(a, "-name"))
		...
		return(mk(status, (struct anode *)atoi(b), (struct anode *)s));
	else if(EQ(a, "-perm")) {
		for(i=0; *b ; ++b) {
			if(*b=='-') continue;
			i <<= 3;
...
status(p)
register struct { int f, st, s; } *p;
{
	Statusqueried = 1;
	return scomp(Status, p->st, p->s);
}
errchk()
{
	if(Status && !Statusqueried) {
		fflush(stdout);
		fprintf(stderr, "find: ");
		perror(Pathname);
	}
