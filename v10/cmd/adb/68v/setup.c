 ...
 * things in struct exec are in vax (!!) byte order
 */
#include "defs.h"
#include "space.h"
#include "map.h"
...
setsym()
{
	...
	magic = swal(hdr.a_magic);
	entry = swal(hdr.a_entry);
	mp->b = PGSIZE;
	txtsize = swal(hdr.a_text)+sizeof(hdr);
	mp->e = txtsize + mp->b;
	mp->f = 0;
	mp->sp = INSTSP;
	mp->flag = MPINUSE;
	mp++;
	datbase = round((WORD)swal(hdr.a_text)+sizeof(hdr), (WORD)PGSIZE) + PGSIZE;
	mp->b = datbase;
	mp->e = datsize = datbase + swal(hdr.a_data);
	mp->f = sizeof(hdr) + swal(hdr.a_text);
	mp->sp = DATASP;
	mp->flag = MPINUSE;
	mp++;
	mp->flag = 0;
