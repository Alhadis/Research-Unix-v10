 ...
 */
	...
	if ( fam_data[i].size == s  &&  strncmp(f, fam_data[i].name, L_FNAME) == 0 )
	    break;
 ...
 */
    ...
    sprintf(name, "%s/%.10s.%d", rastdir, f, s);
    if ( (fd = open(name, 0)) == -1 )  {
	sprintf(name, "%s/%.10s.r%d", rastdir, f, s);
	if ( (fd = open(name, 0)) == -1 )
	    error(FATAL, "can't open raster file for font %.10s, size %d", f, s);
    }	/* End if */
	...
	error(FATAL, "can't read raster file %.10s", name);
 ...
 */
	...
	error(FATAL, "glyph %d out of range for font %.10s", c, fam->name);

    if ( fam->fam[angle] == -1 || fam->fam[angle] + c/128 != last_fam )  {	/* new family */
	if ( fam->fam[angle] == -1 )  {
	...
	if ( (map_size = glyph.height * ((glyph.width + BYTE - 1) / BYTE)) > 0 )  {
		...
		putc(*glyph.bptr++, fp);
	} else error(NON_FATAL, "Bad glyph - font %.10s, glyph %d", fam->name, c);
    }	/* End if */
