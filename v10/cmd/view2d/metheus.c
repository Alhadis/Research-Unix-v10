  ...
  int mapr[],mapg[],mapb[]; /* colors, 0 through maxi */
{
  int i;
  mcmd(CMSEL); mbyte(0); mbyte(0);	/* write in cmap buffer 0 */
  mcmd(CMLDM); mword(0);mword(maxi+2);	/* load entries at loc. 0 */
  mbyte(0);mbyte(0);mbyte(0);  /* index 0 is for black background */
  for(i=0; i<=maxi; i++){
...
{
  mcmd(SYNCH);mword(0); /* next instruction should happen during vert retrace */
...
cvmap ( map, maxi, mapr,mapg,mapb )  /* load color map */
  ...
  int mapr[],mapg[],mapb[]; /* colors, 0 through maxi */
{
  ...
  switch(*map){
    ...
    case 'i':   /* like g, but inverse video */
      ...
      break;
    case 'n':   /* no-op, use default linear color map */
      return(0);
    default:
    err:
