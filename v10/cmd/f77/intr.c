...
int nargs;
{
	...
	case INTRCONV:
		...
		if( ISCOMPLEX(rettype) && nargs==2)
		{
			...
			if (rettype == TYDREAL
			&& ep->tag == TEXPR
			&& ep->exprblock.opcode == OPCONV
			&& ep->exprblock.vtype == TYREAL
			&& (ep = ep->exprblock.leftp,
					ONEOF(ep->headblock.vtype,
						M(TYDREAL)|M(TYDCOMPLEX)))) {

				/* kludge around pcc1 bug */

				q = (expptr)mktemp(TYREAL, NO);
				putexpr(mkexpr(OPASSIGN, q, ep));
				frtemp(q);
				ep = q;
				}
