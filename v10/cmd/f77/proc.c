...
LOCAL dobss(), docomleng(), docommon(), doentry(), dolblfudge(),
...
/* fudge labels (for ASSIGN stmts that reference yet undefined labels) */

LOCAL dolblfudge()
{
	extern chainp Lblfudgelist;
	register chainp cp;
	register Addrp A;
		...
		if(! (( q = (Namep) (p->datap) )->vdcldone) )
		{
			...
			if(q->vtype == TYCHAR)
			{
				if (q->vclass == CLPROC)
					...
					/* character*(*) */
					if (botched_procs && bugwarn & 1)
						warnb1(
					"old f77 botched references to %s",
							varstr(VL,q->varname));
					q->vleng = (expptr)
			...
			/* Once upon a time, external args caused extra
			 * length args to be passed, in case they were
...
{
	register char *s;
	register struct Dimblock *d;
	register expptr e;
	int i, n;
	ftnint type;

	v->nlmemno = ++lastvarno;
	fprintf(asmfile, LABELFMT, memname(STGINIT, ++lastvarno));
	s = ucvarstr(VL, v->varname, &n);
	putstr(asmfile, s, n);
	preven(ALILONG);
	if ((d = v->vdim) && d->nelt && ISCONST(d->nelt)) {
		fprintf(asmfile, LABELFMT, memname(STGINIT, ++lastvarno));
		prconi(asmfile, TYINT, (ftnint)d->ndim);
		prconi(asmfile, TYINT,
			(ftnint)d->nelt->constblock.Const.ci);
		prconi(asmfile, TYINT,
			(ftnint)d->baseoffset->constblock.Const.ci);
		for(i = 0, n = d->ndim - 1; i < n; i++)
			prconi(asmfile, TYINT, (e = d->dims[i].dimsize)
				? (ftnint)e->constblock.Const.ci : 0L);
		}
	fprintf(asmfile, LABELFMT, memname(STGINIT, v->nlmemno));
	praddr(asmfile, STGINIT, v->nlmemno+1, 0L);
	praddr(asmfile, v->vstg, v->vardesc.varno, v->voffset);
	praddr(asmfile, d ? STGINIT : STGNULL, lastvarno, 0L);
	if ((type = v->vtype) == TYCHAR)
		type = -v->vleng->constblock.Const.ci;
	prconi(asmfile, TYINT, type);
	}
...
Namep np;
{
	...
	for(q = np->varxptr.namelist ; q ; q = q->nextp) {
		v = (Namep)q->datap;
		praddr(asmfile, STGINIT, v->nlmemno, 0L);
		}
	fprintf(asmfile, LABELFMT, memname(STGINIT, ++lastvarno));
	s = ucvarstr(VL, np->varname, &len);
	putstr(asmfile, s, len);
	preven(ALILONG);
	fprintf(asmfile, LABELFMT, memname(STGINIT, np->vardesc.varno));
	praddr(asmfile, STGINIT, dimno+1, 0L);
	praddr(asmfile, STGINIT, dimno, 0L);
	prconi(asmfile, TYINT, n);
	}

LOCAL docommon()
{
