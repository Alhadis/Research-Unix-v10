...
LOCAL letter(), conspower(), consbinop(), zdiv();
	...
	case TCONST:
		if(e->constblock.vtype == TYCHAR)
		{
			e->constblock.Const.ccp =
				...
				/*was 1+strlen(e->constblock.Const.ccp) */
			    e->constblock.Const.ccp);
			e->constblock.vleng =
...
register struct Primblock *p;
{
	...
	if (replaced && bugwarn == 1 && (p->argsp || p->namep->vdim))
		warnb1("old f77 complained at stmt func arg %s",
			varstr(VL, np->varname));
	if (!replaced || bugwarn & 2)
