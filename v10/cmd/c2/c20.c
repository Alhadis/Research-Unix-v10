...
char **argv;
{
	...
	if (nflag) {
		...
		fprintf(stderr,"%d jumps to returns\n", nbrret);
	...
	for (;;) {
	  top:
		...
		case SOBGEQ: case SOBGTR: case AOBLEQ: case AOBLSS: case ACB:
			p = alloc(sizeof first);
			p->op = op->opcode; p->subop = op->opcode>>8; p->code=0; cp1=curlp;
		...
		case MOVA:
			p=alloc(sizeof first);
			p->op=op->opcode; p->subop=op->opcode>>8; p->code=0; cp1=curlp+1;
		...
		if (p->op==CASE) {
			...
			do {
				...
				p = alloc(sizeof first); p->op = JSW; p->subop=0; p->code = 0;
...
/*
 * remember the first .set label that
 * looks like a stack offset size
 * and has value 0
 * (so we can stamp out subl2 $L.SO4,sp when L.SO4 is 0)
 */
...
setzerlab()
{
	...
	while (*p && *p != ',')
		p++;
	if (*p++ == 0)
		return;		/* .set without value? */
	len = p - curlp - 1;
	if (len > sizeof(zerlab) - 1)
		return;		/* label too long -- unlikely */
	if (*p++ != '0')
		return;
	if (*p)		/* ,0 or ,0x0 */
		if (*p++ != 'x' || *p++ != '0' || *p)
			return;
	strncpy(zerlab, curlp, len);
	zerlab[len] = 0;	/* probably needless */
	zerlablen = len;
	if (debug)
		printf("Zero label: %s\n", zerlab);
}
...
iterate()
{
	...
	for (p = first.forw; p!=0; p = p->forw) {
		if ((p->op==JBR||p->op==CBR||p->op==JSW) && p->ref) {
			...
			if (p->op == JBR
			&&  rp->op==JBR && (rp->subop==RET||rp->subop==RSB)) {
				nbrret++;
				p->labno = rp->labno;	/* probably meaningless */
				p->subop = rp->subop;
				p->ref = rp->ref;
				p->pop = rp->pop;
				nchange++;
