 ...
 *
 * `UNIT *' in the file is a bad mistake; see openpfile below.
 */
struct hdr {
	UNIT	*lowpc, *highpc;
	int	ncount;
...
getsymtab()
{
	...
	for (i = xbuf.a_syms; i > 0; i -= sizeof(struct nlist)) {
		...
		nlen=strlen(npe->name);
		if(nlen<2 || npe->name[nlen-2]!='.' || npe->name[nlen-1]!='o'){
			npe++;
			nname++;
		}
	}
...
/*
 * beware the scaling below, to calculate highpc and lowpc.
 * it attempts to turn the pointers into something real.
 * the sign bit gets botched and unbotched again.
 * in truth, UNIT * should be some well-defined integer offset instead.
 */
openpfile(filename)
	char *filename;
{
	...
	lowpc = (h.lowpc - (UNIT *)0) * sizeof(UNIT);
	highpc = (h.highpc - (UNIT *)0) * sizeof(UNIT);
	nsamples = (stb.st_size-sizeof(struct hdr)-h.ncount*sizeof(struct cnt))
...
readcntrs()
{
	...
	/* eliminate zero counters */
	if (h.ncount) {
		kp = &cbuf[h.ncount - 1];
		while (kp->cvalue == 0 && kp > cbuf)
			kp--;
		h.ncount = kp-cbuf+1;
	}
	/* sort counters */
...
asgncntrs()
{
	...
	while (--np>=nl) {
		...
		if (kp->cvalue >= np->value) {
			do {
				--kp;
			} while (kp >= cbuf && kp->cvalue >= np->value);
			np->ncall += (kp+1)->cncall;
		}
	}
}
