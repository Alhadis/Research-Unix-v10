/* a0xtrp.f -- translated by f2c (version of 5 February 1993  1:37:14).
...
/* Subroutine */ int a0xtrp_(tm, m, nvar, ng, kmax, xpoly, t, error, ebest, 
	rhg, emag, esave)
real *tm;
integer *m, *nvar;
real *ng;
...
/* Computing MIN */
	...
	mmj = *m - j;
	rhg[j] = ng[*m] / ng[mmj];
	emag[j] = (float)1. / (rhg[j] - (float)1.) + (float)1.;
	if (*xpoly) {
	    rhg[j] += (float)-1.;
	}
/* L30: */
