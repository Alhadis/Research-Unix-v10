/* iodev.f -- translated by f2c (version of 5 February 1993  1:37:14).
...
/* Subroutine */ int iodev_(j, f, r, i, l)
integer *j;
real *f, *r;
integer *i;
logical *l;
{
    ...
    static real gamma = (float)1.;
    static real delta = (float)0.;
...
/* /6S */
/*        IF (TEMP1) CALL SETERR(24H IODEV - J OUT OF BOUNDS, 24, 1, 2) */
/* /7S */
    if (temp1) {
	seterr_(" IODEV - J OUT OF BOUNDS", &c__24, &c__1, &c__2, 24L);
    }
/* / */
    if (*j >= 0) {
	goto L56;
    ...
    n[1] = sqrt((float)2.) * (real) n[0];
/* EXPORT N(ABS(J)-4000) */
/* ONLY N(1) IS GIVEN, USE SQRT(2) INCREASE. */
    if (n[1] == n[0]) {
	++n[1];
    }
    n[2] = sqrt((float)2.) * (real) n[1];
    if (n[2] == n[1]) {
	++n[2];
    }
    n[3] = 0;
