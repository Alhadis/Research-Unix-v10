...
extern struct map kernelmap[], swapmap[];
	...
	struct proc *p;
{
	...
	*(int *)&proto = PG_URKR;
	 ...
	 */
	if (xp->x_flag & XLOAD)
		vinifod((struct fpte *)tptopte(p, 0), xp->x_iptr,
		    (daddr_t)1, xp->x_size);
	else
	...
	struct proc *p;
{
	...
	rmfree(swapmap, ctod(vusize(p)), p->p_swaddr);
	...
	struct proc *p;
{
	...
	p->p_swaddr = srmalloc(swapmap, ctod(vusize(p)));
	return (p->p_swaddr);
}
