...
vsexpand(vssize, dmp, canshrink)
	register clicks_t vssize;
	register struct dmap *dmp;
{
	register int blk = dmmin;
	register int vsbase = 0;
	register swblk_t *ip = dmp->dm_map;
	clicks_t oldsize = dmp->dm_size;
	clicks_t oldalloc = dmp->dm_alloc;

	while (vsbase < oldalloc || vsbase < vssize) {
		if (vsbase >= oldalloc) {
			*ip = srmalloc(swapmap, ctod(blk));
			if (*ip == 0) {
				dmp->dm_size = vsbase;
	...
	struct text *xp;
{
	...
	for (vsbase = 0; vsbase < xp->x_size; vsbase += dmtext) {
		...
		if (blk > dmtext)
			blk = dmtext;
		if ((*dp++ = srmalloc(swapmap, blk)) == 0) {
			vsxfree(xp, vsbase);
			return (0);
		}
	}
	if (xp->x_flag & XPAGI) {
		xp->x_ptdaddr = srmalloc(swapmap, clrnd(ctopt(xp->x_size)));
		if (xp->x_ptdaddr == 0) {
