...
number:
		...
		case TSTAR:
			...
			star++;
			break;
		case TERROR:				/* adb -- graceful error response */
			return(-1);			/* adb */
		}
		tok = scan(&bufp);
	}
...
sender(str, mesg)
	char *str;
{
	/* adb:  modified to accept 'any' command, which I prefer */
	char linebuf[LINESIZE];
	register FILE *ibuf;
	char *allnet = value("allnet");

	if( allnet ) {
		if( strcmp(allnet, "any") == 0) {
			ibuf = setinput(&message[mesg-1]);
			if (readline(ibuf, linebuf) <= 0)
				return(0);
			return substr(linebuf, str) != -1;
		}
		if( strcmp(allnet, "header") == 0 )
			return samebody(str, nameof(&message[mesg-1]));
	}
