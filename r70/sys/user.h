...
#define	SHSIZE	32
#define	NOGROUP	(short)-1		/* endmarker */
/* should NGROUPS be here? */
 
struct	user
{
	struct	pcb u_pcb;
	int	junk1;			/* temp; pcb shrank */
...
#define	SEGUINST	2
	...
	short	u_groups[NGROUPS];	/* array of groups really looked at */
...
#define	r_val2	u_rv.R_val2
	...
	llong_t	u_offset;		/* offset in file for IO */
	struct	inode *u_cdir;		/* pointer to inode of current directory */
	struct	inode *u_rdir;		/* root directory of current process */
	char	junk2[14];		/* used to be current pathname component */
	char	junk4[14+2];		/* used to be current directory entry */
...
#define	ux_relflg	Ux_A.Ux_relflg
	...
	char	u_comm[14];		/* needs to be >= acct.ac_comm */
	time_t	u_start;
	char	u_acflag;
	short	junk6;		/* was u_fpflag */
	short	u_cmask;		/* mask for file creation */
	clicks_t	u_tsize;		/* text size (clicks) */
	clicks_t	u_dsize;		/* data size (clicks) */
	clicks_t	u_ssize;		/* stack size (clicks) */
	...
	clicks_t	u_odsize, u_ossize;	/* for (clumsy) expansion swaps */
