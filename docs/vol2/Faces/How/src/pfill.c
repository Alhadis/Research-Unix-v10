...
{	Node *hunt;
	...
	yy = (int *) malloc(cnt * sizeof(int));	/* max # intersections */
	if (!xx || !yy)
...
/* scale */
		for (x = 0; x < cnt-1; x += 2)
		{	if (x == 0)
				yy[0]   = xx[0] - (xx[0]*Factor)/100;
			else
				yy[x]   = xx[x] - ((xx[x]-xx[x-1])*Factor)/200;
			if (x+1 == cnt-1)
				yy[x+1] = xx[x+1] + ((SZ - xx[x+1])*Factor)/100;
			else
				yy[x+1] = xx[x+1] + ((xx[x+2]-xx[x+1])*Factor)/200;
		}
		for (x = 0; x < cnt; x++)
			xx[x] = yy[x];
/* end scale */
		...
		{	doline(xx[x], xx[x+1], SZ-y, partial, total);
			partial += xx[x+1] - xx[x];
	}	}
	free(xx);
}
