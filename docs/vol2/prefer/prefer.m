...
list.pre "\&", list.format 1,
list.post "\ ",
if "@\*(l5@@",
	set string l5 "nnnnnn",
store width l2 l5,
tab.stops "\n(l2u",
list.spacing off, list on;
...
.ds l5 |cw<spectrum>n
|begin(text_list)
|item{"\f(CWatt\fP"}|small(AT&T) Bell Laboratories
|item{"\f(CWacm\fP"}Association for Computing Machinery
|item{"\f(CWapa\fP"}American Psychological Association
|item{"\f(CWieee\fP"}Institute of Electrical and Electronics Engineers, Inc.
|item{"\f(CWlsa\fP"}Linguistic Society of America
|item{"\f(CWpami\fP"}Pattern Analysis and Machine Intelligence
|item{"\f(CWspectrum\fP"}|small(IEEE) Spectrum
|end(text_list)
...
another |cw(%also) attribute,
surrounding it with optional text
'ne 2v
...
.ds l5 |cw(\-k) |i(maxkey)n
|begin(text_list)
|blank_space(1)
|item{"\f(CW\-c\fP \fIcfile\fP"}specifies a file
of common words, one per line,
to exclude as indexing keys
(default is |cw(/usr/lib/eign)).
|blank_space(1)
|item{"\f(CW\-i\fP \fIifile\fP"}specifies a file
...
|blank_space(1)
|item{"\f(CW\-k\fP \fImaxkey\fP"}specifies the maximum number
of search keys
collected per reference (default is 100 keys).
|blank_space(1)
|item{"\f(CW\-l\fP \fIkeylen\fP"}specifies the maximum length of a search
key(default is 6 characters).
|blank_space(1)
|item{"\f(CW\-v\fP"}tells |i(pinvert) to print statistics.
|end(text_list)
...
.ds l5 |cw(\-s) |i(sname)n
|begin(text_list)
|blank_space(1)
|item{"\f(CW\-p\fP \fIdbfile\fP"}specifies a bibliography
database, |i(dbfile) (default is |cw(prefer.out))
|blank_space(1)
|item{"\f(CW\-s\fP \fIsname\fP"}selects formatting style |i(sname)
(default is |cw(att)). The argument |i(sname)
names of one of the provided styles or
a user-defined style.
|blank_space(1)
|item{"\f(CW\-o\fP \fIokey\fP"}specifies how
...
|remember(stylesec).
|blank_space(1)
|item{"\f(CW\-n\fP"}format using |i(nroff) rather than for |i(troff),
the default.
|blank_space(1)
|item{"\f(CW\-r\fP"}format as
...
Sharon Murrel and Ted Kowalski
